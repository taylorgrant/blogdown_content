...
```{r, echo = FALSE}
library(highcharter)
chart19 <- hchart(strava_19, "heatmap", hcaes(x = month, y = week_ct, value = round(distance, 2))) %>% 
...
chart18 <- hchart(strava_18, "heatmap", hcaes(x = month, y = week_ct, value = round(distance, 2))) %>% 
  hc_colorAxis(minColor = "#d9d9d9", maxColor = "#238b45") %>% 
  hc_xAxis(title = list(text = "")) %>%
  hc_yAxis(title = list(text = "Week of Month"),
           reversed = TRUE,
...
chart17 <- hchart(strava_17, "heatmap", hcaes(x = month, y = week_ct, value = round(distance, 2))) %>% 
  hc_colorAxis(minColor = "#d9d9d9", maxColor = "#238b45") %>% 
  hc_xAxis(title = list(text = "")) %>%
  hc_yAxis(title = list(text = "Week of Month"),
           reversed = TRUE,
