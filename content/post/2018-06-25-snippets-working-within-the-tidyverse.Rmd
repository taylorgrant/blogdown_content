...
The [helper functions](https://dplyr.tidyverse.org/reference/select_helpers.html){target="_blank"} for `select()` are also useful for selecting variables. Though note that when using helpers such as `contains()` that you can only include one term. For instance, this will work.
...
If you want to match across multiple expressions, the `matches()` function will do the trick. 
