...
For this exercise my source of data is going to be the [PGA Tour]("https://www.pgatour.com/") website and its annual statistics. Not only are there literally hundreds of links to tour stats, but these stats range across multiple years and decades. Each statistic has its own url, and each stat-year has its own path. 
...
So now that we have the completed directory, we just need to put it to work. This will require one last function. The `pull_data` function will take each stat-year link from our directory and then capture the html table containing the statistics for that year. We're also going to pass the year through so that our output will include a year column.  
...
When we unnest this tibble, we'll find more nested data. This is actually useful if you want to only keep a select number of years, and want to see how much data is available within each year. 
