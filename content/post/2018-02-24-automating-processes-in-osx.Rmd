...
All current jobs on the computer can be seen using the command `launchctl list`, or you can call your job specifically using `launchctl list | grep com.LABEL_NAME`. To the left of each plist job is a Status code. 0 status means everything is working, anything else probably means something is wrong. If the job returns a non-zero status, you can look up the code in terminal using `launch ctl #` where # is the Status code. 
